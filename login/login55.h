...
class LOGIN55 : public LOGIN
{
public:
    LOGIN55(){}
    LOGIN55(SERVER_CONNECTION_CNF* scc, PROTOCOL* pl) :
    scc(scc), pl(pl), client_capabilities(0)
    {
    }
    ...
    char * get_41_lenc_string(char **buffer, size_t *max_bytes_available,
            size_t *string_length);
    unsigned long parse_client_handshake_packet(unsigned char **buff, unsigned long pkt_len);
    bool check_login();
